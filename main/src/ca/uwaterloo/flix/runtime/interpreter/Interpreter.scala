...
object Interpreter extends Phase[Root, Array[String] => Int] {
  ...
  def run(root: Root)(implicit flix: Flix): Validation[(Array[String]) => Int, CompilationError] = flix.phase("Interpreter") {
    ...
    r match {
      case Some(value) => value.toSuccess
      case None => Validation.Failure(LazyList(new InterpreterError("no main", SourceLocation.Unknown)))
    }
  }
  ...
  private def evalBinary(sop: SemanticOperator, exp1: Expression, exp2: Expression, env0: Map[String, AnyRef], lenv0: Map[Symbol.LabelSym, Expression], root: Root, currentLabel: KLabel)(implicit flix: Flix): AnyRef = {
    ...
    def evalInt64Op(sop: SemanticOperator.Int64Op): AnyRef = {
      ...
      sop match {
        ...
        case SemanticOperator.Int64Op.Exp => Value.Int64(math.pow(cast2int64(v1).toDouble, cast2int64(v2).toDouble).toLong)
  ...
  def reduceK(fromLabel: KLabel, toLabel: KLabel, value: AnyRef, con: MonoType)(implicit loc: SourceLocation): AnyRef = (value, con) match {
    ...
    case (v, MonoType.WildCard) => println("unsafe reduceK"); v
  ...
  class InterpreterError(msg: String, locc: SourceLocation) extends CompilationError {
    /**
      * Returns the kind of error message, e.g. "Syntax Error" or "Type Error".
      */
    override def kind: String = "Interpreter Error"

    /**
      * Returns a short description of the error message.
      */
    override def summary: String = msg

    /**
      * Returns the formatted error message.
      */
    override def message: VirtualTerminal = new VirtualTerminal().<<(msg)
